CloudPebble.Editor = (function() {
    ...
    var run = function() {
        ...
        CloudPebble.Editor.SaveAll().then(function() {
            return CloudPebble.Compile.RunBuild(true)
    ...
    var edit_source_file = function(file, show_ui_editor) {
        ...
        return Ajax.Get('/api/load-source-file.lua?file_path=' + encodeURIComponent(file.file_path)).then(function(data) {
            ...
            function file_kind_in(options) {
                return options.includes(file_kind);
            }
            ...
            run_btn.popover({
                ...
                content: function() {
                    ...
                    if (build_platforms) {
                        var build_platform_names = build_platforms.split(',').map(capitalise_first_letter).join(', ');
    ...
    var save_all = function() {
        return Promise.all(open_codemirrors.map(cm => cm.cloudpebble_save));
    };
    ...
    function create_remote_file(params) {
        ...
        return Ajax.Post("/api/create-source-file.lua", params).then(function(data) {
    ...
    function init_create_prompt() {
        ...
        function get_default_js_name() {
            ...
            for (let spec of required_js) {
                if (!spec.project_type || CloudPebble.ProjectInfo.type == spec.project_type) {
                    if (current_target == spec.target && !project_source_files.some(x => x.target==spec.target && x.name.endsWith('.js'))) {
        ...
        function get_js_target(selector) {
            ...
            if (!target) {
                target = ['native', 'package'].includes(CloudPebble.ProjectInfo.type) ? 'pkjs': 'app';
            }
            return target;
        }
