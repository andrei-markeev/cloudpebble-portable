...
CloudPebble.ProjectProperties = (function() {
    ...
    var obj = {
        Init: function() {
            for (let [property, types] of Object.entries(spec)) {
                Object.defineProperty(obj, property, {
                    value: types.includes(CloudPebble.ProjectInfo.type)
                });
            }
        }
    };
...
CloudPebble.GlobalShortcuts = (function() {
    var make_shortcut_checker = function (command) {
        if (!(command.indexOf('-') > -1)) {
            let found = undefined;
            for (let [key, value] of Object.entries(CodeMirror.keyMap.default)) {
                if (value === command) {
                    found = key;
                    break;
                }
    ...
    $(document).keydown(function (e) {
        if (!e.isDefaultPrevented()) {
            for (let shortcut of global_shortcuts) {
    ...
    return {
        SetShortcutHandlers: function (shortcuts) {
            const new_shortcuts = [];
            for (let [key, func] of Object.entries(shortcuts)) {
                new_shortcuts.push({
                    checker: make_shortcut_checker(key),
                    func: func
                });
            }
            global_shortcuts = [...global_shortcuts, ...new_shortcuts];
        }
    }
})();
